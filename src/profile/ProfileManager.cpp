                                   ...
                                   QHash<Profile::Property, QVariant> propertyMap, bool persistent)
{
    ...
    if (persistent && isFallback) {
        ...
        // ProfileList listens to the profileRemoved signal to update the profile
        // menu actions (which is used by the "Switch Profile" context menu). This
        // is needed to prevent double entries for the newly added profile.
        emit profileRemoved(_fallbackProfile);

        // Since the profile object pointed to by _fallbackProfile has been given
        // a name above, now init a fallback profile again. This way there is always
        // a "Default" profile available in the context menu.
        initFallbackProfile();
    }
    ...
    for (auto it = propertyMap.cbegin(); it != propertyMap.cend(); ++it) {
        ...
        // in the name box in the edit profile dialog; i.e. saving the fallback
        // profile where the user didn't change the name at all, the uniqueProfileName
        // is used silently a couple of lines above.
        if ((property == Profile::Name || property == Profile::UntranslatedName)
            && value == QLatin1String("Default")) {
            value = uniqueProfileName;
            if (!messageShown) {
                KMessageBox::sorry(nullptr,
                                   i18n("The name \"Default\" is reserved for the built-in fallback profile;\n"
                                        "the profile is going to be saved as \"%1\"", uniqueProfileName));
                messageShown = true;
            }
        }
