...
void EditProfileDialog::save()
{
    ...
    if (isFallback) {
        // Needed to update the profile name in the dialog, if the user used
        // "Apply" and the dialog is still open, since the fallback profile will
        // have a unique name, e.g. "Profile 1", generated for it by the ProfileManager.
        setProfile(_profile);
    }
